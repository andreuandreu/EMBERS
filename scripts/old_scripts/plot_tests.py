...
import scripts.old_scripts.tests_numerical_analytical as tna
...
def plot_tau_N_dep():
    ...
    ax.plot(N0_e_values, tau2_values, lw = 2.5, color = 'orange', ls = '--', label=r'$\Delta t^{max}$ = 60[yr]')
...
def plot_tau_N_dep_2gradients():
    ...
    ax.plot(N0_e_values, tau1_values, lw = 2.5, color = 'm', label=r'$\Delta t^{max}$ = 1[yr]')
    ax.plot(N0_e_values, tau2_values, lw = 2.5, color = 'orange', ls = '--', label=r'$\Delta t^{max}$ = 60[yr]')
...
def plot_exp_dec(tau, E0):
    ...
    fig, ax = plt.subplots()
    plt.annotate('$\Delta t^{max}$', xy=(t_death/2, E0-3), fontsize = 13)
    ...
    ax.set_ylabel('$\eta^{max}$', fontsize = 10)
...
def plot_reconstructed_sequence(t_series, E_series, events, periode, noiseLevel, pop, halfLife):
    ...
    ax1.set_title('$T =$' + "{:.2f}".format(periode) +
                  '[yr] $\nu =$' + "{:.0f}".format(noiseLevel*100) + '%' +
                  ' $\eta^{max} =$' + "{:d}".format(pop) +
